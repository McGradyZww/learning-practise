...
# 原子_交易期货_同步接口_获取同步时间
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_tdfusyn_GetSyncTime;;
DELIMITER ;;
CREATE PROCEDURE pra_tdfusyn_GetSyncTime(
    IN p_opor_co_no int,
    IN p_opor_no int,
...
# 原子_交易期货_同步接口_更新同步时间
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_tdfusyn_UpdateSyncTime;;
DELIMITER ;;
CREATE PROCEDURE pra_tdfusyn_UpdateSyncTime(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_交易期货_同步接口_同步交易组业务资金表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_tdfusyn_Synctdfuac_exgp_busi_capit;;
DELIMITER ;;
CREATE PROCEDURE pra_tdfusyn_Synctdfuac_exgp_busi_capit(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* from (select * from tb_tdfuac_exgp_busi_capit where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_tdfuac_exgp_busi_capit where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_交易期货_同步接口_同步资产账户业务资金表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_tdfusyn_Synctdfuac_asac_busi_capit;;
DELIMITER ;;
CREATE PROCEDURE pra_tdfusyn_Synctdfuac_asac_busi_capit(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* from (select * from tb_tdfuac_asac_busi_capit where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_tdfuac_asac_busi_capit where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_交易期货_同步接口_同步交易组业务持仓表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_tdfusyn_Synctdfuac_exgp_busi_posi;;
DELIMITER ;;
CREATE PROCEDURE pra_tdfusyn_Synctdfuac_exgp_busi_posi(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* set @同步字段2# = ''; */
    set v_sync_field2 = '';
    select GROUP_CONCAT(row_id),GROUP_CONCAT(create_date),GROUP_CONCAT(create_time),GROUP_CONCAT(update_date),GROUP_CONCAT(update_time),GROUP_CONCAT(update_times),GROUP_CONCAT(co_no),GROUP_CONCAT(pd_no),GROUP_CONCAT(exch_group_no),GROUP_CONCAT(asset_acco_no),GROUP_CONCAT(exch_crncy_type),GROUP_CONCAT(exch_no),GROUP_CONCAT(futu_acco_no),GROUP_CONCAT(contrc_code_no),GROUP_CONCAT(contrc_type),GROUP_CONCAT(contrc_unit),GROUP_CONCAT(lngsht_type),GROUP_CONCAT(hedge_type),GROUP_CONCAT(comb_code),GROUP_CONCAT(begin_qty),GROUP_CONCAT(curr_qty),GROUP_CONCAT(frozen_qty),GROUP_CONCAT(unfroz_qty),GROUP_CONCAT(comm_frozen_qty),GROUP_CONCAT(comm_unfroz_qty),GROUP_CONCAT(comm_capt_qty),GROUP_CONCAT(comm_releas_qty),GROUP_CONCAT(trade_frozen_qty),GROUP_CONCAT(trade_unfroz_qty),GROUP_CONCAT(trade_capt_qty),GROUP_CONCAT(trade_releas_qty),GROUP_CONCAT(strike_releas_qty),GROUP_CONCAT(pre_comm_capt_qty),GROUP_CONCAT(today_comm_capt_qty),GROUP_CONCAT(old_trade_capt_qty),GROUP_CONCAT(today_trade_capt_qty),GROUP_CONCAT(pre_strike_capt_qty),GROUP_CONCAT(today_strike_capt_qty),GROUP_CONCAT(rece_margin),GROUP_CONCAT(pre_settle_price),GROUP_CONCAT(realize_pandl),GROUP_CONCAT(close_pandl),GROUP_CONCAT(open_amount)

    /* into @同步字段1#,@同步字段2#,@同步字段3#,@同步字段4#,@同步字段5#,@同步字段6#,@同步字段7#,@同步字段8#,@同步字段9#,@同步字段10#,@同步字段11#,@同步字段12#,@同步字段13#,@同步字段14#,@同步字段15#,@同步字段16#,@同步字段17#,@同步字段18#,@同步字段19#,@同步字段20#,@同步字段21#,@同步字段22#,@同步字段23#,@同步字段24#,@同步字段25#,@同步字段26#,@同步字段27#,@同步字段28#,@同步字段29#,@同步字段30#,@同步字段31#,@同步字段32#,@同步字段33#,@同步字段34#,@同步字段35#,@同步字段36#,@同步字段37#,@同步字段38#,@同步字段39#,@同步字段40#,@同步字段41#,@同步字段42#,@同步字段43# */
    into v_sync_field1,v_sync_field2,v_sync_field3,v_sync_field4,v_sync_field5,v_sync_field6,v_sync_field7,v_sync_field8,v_sync_field9,v_sync_field10,v_sync_field11,v_sync_field12,v_sync_field13,v_sync_field14,v_sync_field15,v_sync_field16,v_sync_field17,v_sync_field18,v_sync_field19,v_sync_field20,v_sync_field21,v_sync_field22,v_sync_field23,v_sync_field24,v_sync_field25,v_sync_field26,v_sync_field27,v_sync_field28,v_sync_field29,v_sync_field30,v_sync_field31,v_sync_field32,v_sync_field33,v_sync_field34,v_sync_field35,v_sync_field36,v_sync_field37,v_sync_field38,v_sync_field39,v_sync_field40,v_sync_field41,v_sync_field42,v_sync_field43

    /* from (select * from tb_tdfuac_exgp_busi_posi where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_tdfuac_exgp_busi_posi where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_交易期货_同步接口_同步资产账户业务持仓表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_tdfusyn_Synctdfuac_asac_busi_posi;;
DELIMITER ;;
CREATE PROCEDURE pra_tdfusyn_Synctdfuac_asac_busi_posi(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* set @同步字段2# = ''; */
    set v_sync_field2 = '';
    select GROUP_CONCAT(row_id),GROUP_CONCAT(create_date),GROUP_CONCAT(create_time),GROUP_CONCAT(update_date),GROUP_CONCAT(update_time),GROUP_CONCAT(update_times),GROUP_CONCAT(co_no),GROUP_CONCAT(pd_no),GROUP_CONCAT(asset_acco_no),GROUP_CONCAT(exch_crncy_type),GROUP_CONCAT(exch_no),GROUP_CONCAT(futu_acco_no),GROUP_CONCAT(contrc_code_no),GROUP_CONCAT(contrc_type),GROUP_CONCAT(contrc_unit),GROUP_CONCAT(lngsht_type),GROUP_CONCAT(hedge_type),GROUP_CONCAT(comb_code),GROUP_CONCAT(begin_qty),GROUP_CONCAT(curr_qty),GROUP_CONCAT(frozen_qty),GROUP_CONCAT(unfroz_qty),GROUP_CONCAT(comm_frozen_qty),GROUP_CONCAT(comm_unfroz_qty),GROUP_CONCAT(comm_capt_qty),GROUP_CONCAT(comm_releas_qty),GROUP_CONCAT(trade_frozen_qty),GROUP_CONCAT(trade_unfroz_qty),GROUP_CONCAT(trade_capt_qty),GROUP_CONCAT(trade_releas_qty),GROUP_CONCAT(strike_releas_qty),GROUP_CONCAT(pre_comm_capt_qty),GROUP_CONCAT(today_comm_capt_qty),GROUP_CONCAT(old_trade_capt_qty),GROUP_CONCAT(today_trade_capt_qty),GROUP_CONCAT(pre_strike_capt_qty),GROUP_CONCAT(today_strike_capt_qty),GROUP_CONCAT(rece_margin),GROUP_CONCAT(pre_settle_price),GROUP_CONCAT(realize_pandl),GROUP_CONCAT(close_pandl),GROUP_CONCAT(open_amount)
    ...
    /* from (select * from tb_tdfuac_asac_busi_posi where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_tdfuac_asac_busi_posi where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_交易期货_同步接口_同步交易组持仓明细表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_tdfusyn_Synctdfuac_exgp_posi_detail;;
DELIMITER ;;
CREATE PROCEDURE pra_tdfusyn_Synctdfuac_exgp_posi_detail(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* set @同步字段2# = ''; */
    set v_sync_field2 = '';
    select GROUP_CONCAT(row_id),GROUP_CONCAT(create_date),GROUP_CONCAT(create_time),GROUP_CONCAT(update_date),GROUP_CONCAT(update_time),GROUP_CONCAT(update_times),GROUP_CONCAT(co_no),GROUP_CONCAT(pd_no),GROUP_CONCAT(exch_group_no),GROUP_CONCAT(asset_acco_no),GROUP_CONCAT(exch_no),GROUP_CONCAT(exch_crncy_type),GROUP_CONCAT(futu_acco_no),GROUP_CONCAT(contrc_code_no),GROUP_CONCAT(contrc_unit),GROUP_CONCAT(contrc_type),GROUP_CONCAT(curr_qty),GROUP_CONCAT(invest_type),GROUP_CONCAT(lngsht_type),GROUP_CONCAT(hedge_type),GROUP_CONCAT(open_posi_date),GROUP_CONCAT(open_posi_time),GROUP_CONCAT(open_posi_price),GROUP_CONCAT(open_posi_qty),GROUP_CONCAT(open_amount),GROUP_CONCAT(open_posi_fee),GROUP_CONCAT(close_posi_fee),GROUP_CONCAT(trade_capt_qty),GROUP_CONCAT(strike_capt_qty),GROUP_CONCAT(marked_sett_price),GROUP_CONCAT(strike_no),GROUP_CONCAT(detail_posi_id),GROUP_CONCAT(rece_margin),GROUP_CONCAT(close_pandl),GROUP_CONCAT(marked_pandl)

    /* into @同步字段1#,@同步字段2#,@同步字段3#,@同步字段4#,@同步字段5#,@同步字段6#,@同步字段7#,@同步字段8#,@同步字段9#,@同步字段10#,@同步字段11#,@同步字段12#,@同步字段13#,@同步字段14#,@同步字段15#,@同步字段16#,@同步字段17#,@同步字段18#,@同步字段19#,@同步字段20#,@同步字段21#,@同步字段22#,@同步字段23#,@同步字段24#,@同步字段25#,@同步字段26#,@同步字段27#,@同步字段28#,@同步字段29#,@同步字段30#,@同步字段31#,@同步字段32#,@同步字段33#,@同步字段34#,@同步字段35# */
    into v_sync_field1,v_sync_field2,v_sync_field3,v_sync_field4,v_sync_field5,v_sync_field6,v_sync_field7,v_sync_field8,v_sync_field9,v_sync_field10,v_sync_field11,v_sync_field12,v_sync_field13,v_sync_field14,v_sync_field15,v_sync_field16,v_sync_field17,v_sync_field18,v_sync_field19,v_sync_field20,v_sync_field21,v_sync_field22,v_sync_field23,v_sync_field24,v_sync_field25,v_sync_field26,v_sync_field27,v_sync_field28,v_sync_field29,v_sync_field30,v_sync_field31,v_sync_field32,v_sync_field33,v_sync_field34,v_sync_field35

    /* from (select * from tb_tdfuac_exgp_posi_detail where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_tdfuac_exgp_posi_detail where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
