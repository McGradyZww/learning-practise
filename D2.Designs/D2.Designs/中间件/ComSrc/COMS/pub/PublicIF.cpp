...
API_EXPORT int LD_CALL_MODE QueryInterface(int iFunIndex, LPBIZ_FUNC_INFO bizFuncInfo)
{
    ...
    if (iFunIndex==0)
    {
        strcpy(bizFuncInfo->szFuncName, "pubL.24.1");
        bizFuncInfo->lpFunc = fnFunc0;
    }
    else if (iFunIndex==1)
    {
        strcpy(bizFuncInfo->szFuncName, "pubL.24.2");
...
int iPriority=lpInBizMsg->GetInt32(LDTAG_PRIORITY);
    ...
    lpTSCall1Req1=glpFastMsgFactory->CreateFastMessage("pubT.24.73", 0);
    ...
    {
        lpOutBizMsg->SetInt32(LDTAG_ERRORNO, -1);
        lpOutBizMsg->SetString(LDTAG_ERRORINFO, "[pubT.24.73]subcall timed out!");
        iRet=-2;
        goto ENDSYS;
    }
...
int iPriority=lpInBizMsg->GetInt32(LDTAG_PRIORITY);
    ...
    strncpy(v_func_code, lpInBizMsg->GetString(LDBIZ_FUNC_CODE_STR),16);
    v_func_code[16] = '\0';
    strncpy(v_busi_ctrl_str, lpInBizMsg->GetString(LDBIZ_BUSI_CTRL_STR_STR),64);
    v_busi_ctrl_str[64] = '\0';
...
END:
    ...
    lpOutBizMsg->SetString(LDBIZ_ERROR_PROMPT_STR,v_error_prompt);
    //对输出参数进行赋值处理
    lpOutBizMsg->SetString(LDBIZ_CO_NO_RIGHTS_STR_STR,v_co_no_rights_str);
    lpOutBizMsg->SetString(LDBIZ_PD_NO_RIGHTS_STR_STR,v_pd_no_rights_str);
    lpOutBizMsg->SetString(LDBIZ_EXCH_GROUP_NO_RIGHTS_STR_STR,v_exch_group_no_rights_str);
ENDSYS:
    if(lpTSCall1Ans0)
        lpTSCall1Ans0->FreeMsg();
    if(lpTSCall1Ans1)
