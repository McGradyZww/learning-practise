...
# 原子_产品期货_同步接口_获取同步时间
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_pdfu_GetSyncTime;;
DELIMITER ;;
CREATE PROCEDURE pra_pdfu_GetSyncTime(
    IN p_opor_co_no int,
    IN p_opor_no int,
...
# 原子_产品期货_同步接口_更新同步时间
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_pdfu_UpdateSyncTime;;
DELIMITER ;;
CREATE PROCEDURE pra_pdfu_UpdateSyncTime(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_产品期货_同步接口_同步交易组资金表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_pdfu_Syncpdfuca_exgp_capit;;
DELIMITER ;;
CREATE PROCEDURE pra_pdfu_Syncpdfuca_exgp_capit(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* set @同步字段2# = ''; */
    set v_sync_field2 = '';
    select GROUP_CONCAT(row_id),GROUP_CONCAT(create_date),GROUP_CONCAT(create_time),GROUP_CONCAT(update_date),GROUP_CONCAT(update_time),GROUP_CONCAT(update_times),GROUP_CONCAT(co_no),GROUP_CONCAT(pd_no),GROUP_CONCAT(asset_acco_no),GROUP_CONCAT(exch_group_no),GROUP_CONCAT(crncy_type),GROUP_CONCAT(begin_amt),GROUP_CONCAT(curr_amt),GROUP_CONCAT(capt_margin),GROUP_CONCAT(frozen_amt),GROUP_CONCAT(unfroz_amt),GROUP_CONCAT(total_rece_amt),GROUP_CONCAT(total_payab_amt),GROUP_CONCAT(intrst_rate),GROUP_CONCAT(intrst_base_amt),GROUP_CONCAT(pre_entry_intrst_amt)

    /* into @同步字段1#,@同步字段2#,@同步字段3#,@同步字段4#,@同步字段5#,@同步字段6#,@同步字段7#,@同步字段8#,@同步字段9#,@同步字段10#,@同步字段11#,@同步字段12#,@同步字段13#,@同步字段14#,@同步字段15#,@同步字段16#,@同步字段17#,@同步字段18#,@同步字段19#,@同步字段20#,@同步字段21# */
    into v_sync_field1,v_sync_field2,v_sync_field3,v_sync_field4,v_sync_field5,v_sync_field6,v_sync_field7,v_sync_field8,v_sync_field9,v_sync_field10,v_sync_field11,v_sync_field12,v_sync_field13,v_sync_field14,v_sync_field15,v_sync_field16,v_sync_field17,v_sync_field18,v_sync_field19,v_sync_field20,v_sync_field21

    /* from (select * from tb_pdfuca_exgp_capit where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_pdfuca_exgp_capit where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_产品期货_同步接口_同步交易组资产表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_pdfu_Syncpdfuca_exgp_asset;;
DELIMITER ;;
CREATE PROCEDURE pra_pdfu_Syncpdfuca_exgp_asset(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* set @同步字段2# = ''; */
    set v_sync_field2 = '';
    select GROUP_CONCAT(row_id),GROUP_CONCAT(create_date),GROUP_CONCAT(create_time),GROUP_CONCAT(update_date),GROUP_CONCAT(update_time),GROUP_CONCAT(update_times),GROUP_CONCAT(co_no),GROUP_CONCAT(pd_no),GROUP_CONCAT(exch_group_no),GROUP_CONCAT(crncy_type),GROUP_CONCAT(begin_nav),GROUP_CONCAT(nav_asset),GROUP_CONCAT(cash_asset),GROUP_CONCAT(futu_asset)

    /* into @同步字段1#,@同步字段2#,@同步字段3#,@同步字段4#,@同步字段5#,@同步字段6#,@同步字段7#,@同步字段8#,@同步字段9#,@同步字段10#,@同步字段11#,@同步字段12#,@同步字段13#,@同步字段14# */
    into v_sync_field1,v_sync_field2,v_sync_field3,v_sync_field4,v_sync_field5,v_sync_field6,v_sync_field7,v_sync_field8,v_sync_field9,v_sync_field10,v_sync_field11,v_sync_field12,v_sync_field13,v_sync_field14

    /* from (select * from tb_pdfuca_exgp_asset where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_pdfuca_exgp_asset where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_产品期货_同步接口_同步资产账户资金表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_pdfu_Syncpdfuca_asac_capit;;
DELIMITER ;;
CREATE PROCEDURE pra_pdfu_Syncpdfuca_asac_capit(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* from (select * from tb_pdfuca_asac_capit where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_pdfuca_asac_capit where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_产品期货_同步接口_同步交易组持仓表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_pdfu_Syncpdfupo_exgp_posi;;
DELIMITER ;;
CREATE PROCEDURE pra_pdfu_Syncpdfupo_exgp_posi(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* set @同步字段2# = ''; */
    set v_sync_field2 = '';
    select GROUP_CONCAT(row_id),GROUP_CONCAT(create_date),GROUP_CONCAT(create_time),GROUP_CONCAT(update_date),GROUP_CONCAT(update_time),GROUP_CONCAT(update_times),GROUP_CONCAT(co_no),GROUP_CONCAT(pd_no),GROUP_CONCAT(asset_acco_no),GROUP_CONCAT(exch_group_no),GROUP_CONCAT(exch_no),GROUP_CONCAT(futu_acco_no),GROUP_CONCAT(contrc_code_no),GROUP_CONCAT(contrc_type),GROUP_CONCAT(lngsht_type),GROUP_CONCAT(hedge_type),GROUP_CONCAT(comb_code),GROUP_CONCAT(contrc_unit),GROUP_CONCAT(begin_qty),GROUP_CONCAT(curr_qty),GROUP_CONCAT(frozen_qty),GROUP_CONCAT(unfroz_qty),GROUP_CONCAT(pre_settle_price),GROUP_CONCAT(sett_price),GROUP_CONCAT(rece_margin),GROUP_CONCAT(sett_flag)
    ...
    /* from (select * from tb_pdfupo_exgp_posi where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_pdfupo_exgp_posi where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
...
# 原子_产品期货_同步接口_同步资产账户持仓表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_pdfu_Syncpdfupo_asac_posi;;
DELIMITER ;;
CREATE PROCEDURE pra_pdfu_Syncpdfupo_asac_posi(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* set @同步字段2# = ''; */
    set v_sync_field2 = '';
    select GROUP_CONCAT(row_id),GROUP_CONCAT(create_date),GROUP_CONCAT(create_time),GROUP_CONCAT(update_date),GROUP_CONCAT(update_time),GROUP_CONCAT(update_times),GROUP_CONCAT(co_no),GROUP_CONCAT(pd_no),GROUP_CONCAT(asset_acco_no),GROUP_CONCAT(exch_no),GROUP_CONCAT(futu_acco_no),GROUP_CONCAT(contrc_code_no),GROUP_CONCAT(contrc_type),GROUP_CONCAT(lngsht_type),GROUP_CONCAT(hedge_type),GROUP_CONCAT(comb_code),GROUP_CONCAT(contrc_unit),GROUP_CONCAT(begin_qty),GROUP_CONCAT(curr_qty),GROUP_CONCAT(frozen_qty),GROUP_CONCAT(unfroz_qty),GROUP_CONCAT(pre_settle_price),GROUP_CONCAT(sett_price),GROUP_CONCAT(rece_margin),GROUP_CONCAT(sett_flag)
    ...
    /* from (select * from tb_pdfupo_asac_posi where {更新日期} >= @更新日期# and {更新时间} >= @更新时间#  and {记录序号}> @记录序号# LIMIT @指定行数#) a ; */
    from (select * from tb_pdfupo_asac_posi where update_date >= v_update_date and update_time >= v_update_time  and row_id> v_row_id LIMIT v_row_count) a ;
    END;
