...
# 原子_公共_权限客户端_新增角色
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_AddRole;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_AddRole(
    IN p_opor_co_no int,
    IN p_opor_no int,
...
# 原子_公共_权限客户端_修改角色
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_ModiRole;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_ModiRole(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [更新表记录][公共_权限_角色表][{角色名称}=@角色名称#][{角色编号}=@角色编号# and {机构编号}=@操作员机构编号#][2][@角色编号#,@操作员机构编号#] */
    set v_update_date = date_format(curdate(),'%Y%m%d');
    set v_update_time = date_format(curtime(),'%H%i%s');
    update db_pub.tb_perm_role set update_date = v_update_date, update_time = v_update_time, update_times = update_times + 1, role_name=v_role_name where role_no=v_role_no and co_no=v_opor_co_no;
    if v_error_code = "1" then
                
        SET v_error_code = "pubA.21.2.2";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no);
        end if;
        leave label_pro;
    end if;


    /* [获取表记录变量][公共_权限_角色表][{机构编号},{角色类型}][@机构编号#,@角色类型#][{角色编号}=@角色编号# and {机构编号}=@操作员机构编号#][4][@角色编号#,@操作员机构编号#] */
    select co_no,role_type into v_co_no,v_role_type from db_pub.tb_perm_role where role_no=v_role_no and co_no=v_opor_co_no limit 1;
    if FOUND_ROWS() = 0 then
                
        SET v_error_code = "pubA.21.2.4";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no);
...
# 原子_公共_权限客户端_删除角色
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_DeleteRole;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_DeleteRole(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [获取表记录变量][公共_权限_角色表][字段][字段变量][{角色编号}=@角色编号# and {机构编号}=@操作员机构编号#][4][@角色编号#,@操作员机构编号#] */
    select row_id, create_date, create_time, 
        ...
        v_co_no, v_role_code, v_role_name, v_role_type from db_pub.tb_perm_role where role_no=v_role_no and co_no=v_opor_co_no limit 1;
    if FOUND_ROWS() = 0 then
                
        SET v_error_code = "pubA.21.3.4";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no);
    ...
    /* [删除表记录][公共_权限_角色表][{角色编号}=@角色编号# and {机构编号}=@操作员机构编号#][3][@角色编号#,@操作员机构编号#] */
    delete from db_pub.tb_perm_role 
        where role_no=v_role_no and co_no=v_opor_co_no;
    if v_error_code = 1 then
                
        SET v_error_code = "pubA.21.3.3";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no);
...
# 原子_公共_权限客户端_查询角色列表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryRoleList;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryRoleList(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [获取表全记录][公共_权限_角色表][字段][{机构编号}=@机构编号# and (@角色编号#=0 or {角色编号}=@角色编号#) and ({角色类型}=《角色类型-交易》 or {角色类型}=《角色类型-管理》 or {角色类型}=《角色类型-运维》 or {角色类型}=《角色类型-服务》)] */
    select row_id, create_date, create_time, 
        update_date, update_time, update_times, role_no, 
        co_no, role_code, role_name, role_type from db_pub.tb_perm_role where co_no=v_co_no and (v_role_no=0 or role_no=v_role_no) and (role_type=1 or role_type=2 or role_type=3 or role_type=4);
...
# 原子_公共_权限客户端_批量新增角色菜单功能
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_BatchAddRoleMenuFunc;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_BatchAddRoleMenuFunc(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_批量删除角色菜单功能
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_BatchDeleteRoleMenuFunc;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_BatchDeleteRoleMenuFunc(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_查询角色菜单功能列表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryRoleMenuFunc;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryRoleMenuFunc(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [获取表记录变量][公共_权限_角色表][{角色类型}][@角色类型#][{角色编号}=@角色编号# and {机构编号}=@操作员机构编号#][201][@角色编号#,@操作员机构编号#] */
    select role_type into v_role_type from db_pub.tb_perm_role where role_no=v_role_no and co_no=v_opor_co_no limit 1;
    if FOUND_ROWS() = 0 then
                
        SET v_error_code = "pubA.21.15.201";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("角色编号=",v_role_no,",","操作员机构编号=",v_opor_co_no);
...
# 原子_公共_权限客户端_复制角色
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_CopyRole;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_CopyRole(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [获取表记录变量][公共_权限_角色表][{角色类型}][@角色类型#][{角色编号}=@源角色编号# and {机构编号}=@机构编号#][201][@源角色编号#,@机构编号#] */
    select role_type into v_role_type from db_pub.tb_perm_role where role_no=v_source_role_no and co_no=v_co_no limit 1;
    if FOUND_ROWS() = 0 then
                
        SET v_error_code = "pubA.21.4.201";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("源角色编号=",v_source_role_no,",","机构编号=",v_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("源角色编号=",v_source_role_no,",","机构编号=",v_co_no);
        end if;
        leave label_pro;
    end if;
...
# 原子_公共_权限客户端_查询操作员可用角色列表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryOperatorAllowRoleList;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryOperatorAllowRoleList(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [获取表全记录][公共_权限_角色表][字段][{机构编号}=@操作员机构编号# and ({角色类型}=《角色类型-交易》 or {角色类型}=《角色类型-管理》 or {角色类型}=《角色类型-运维》 or {角色类型}=《角色类型-服务》)] */
    select row_id, create_date, create_time, 
        update_date, update_time, update_times, role_no, 
        co_no, role_code, role_name, role_type from db_pub.tb_perm_role where co_no=v_opor_co_no and (role_type=1 or role_type=2 or role_type=3 or role_type=4);
...
# 原子_公共_权限客户端_新增操作员角色
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_AddOporRole;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_AddOporRole(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [获取表记录变量][公共_权限_角色表][{角色类型}][@角色类型#][{角色编号}=@角色编号# and {机构编号}=@机构编号#][201][@角色编号#,@机构编号#] */
    select role_type into v_role_type from db_pub.tb_perm_role where role_no=v_role_no and co_no=v_co_no limit 1;
    if FOUND_ROWS() = 0 then
                
        SET v_error_code = "pubA.21.22.201";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","机构编号=",v_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("角色编号=",v_role_no,",","机构编号=",v_co_no);
        end if;
        leave label_pro;
    end if;
    ...
    if v_is_check=1 and (v_role_type = 11 or v_role_type = 12 or v_role_type = 13 or v_role_type = 14) then
        ...
        SET v_error_code = "pubA.21.22.219";
        if v_mysql_message <> "" then
...
# 原子_公共_权限客户端_删除操作员角色
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_DeleteOporRole;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_DeleteOporRole(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_查询操作员角色列表
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryOporRoleList;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryOporRoleList(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_获取操作员特殊角色编号
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_GetOporSpecRoleNo;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_GetOporSpecRoleNo(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    if v_role_no=0 and v_is_check = 1  then
        ...
        call db_pub.pra_base_GetCoBusiRecordNo(
            ...
            v_curr_no);
        if v_error_code = "1" then
            SET v_error_code = "pubA.21.25.999";
            if v_mysql_message <> "" then
...
# 原子_公共_权限客户端_查询操作员普通菜单
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryOporAllMenu;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryOporAllMenu(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_查询操作员特殊菜单
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryOporSpecMenu;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryOporSpecMenu(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_查询操作员普通菜单功能
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryOporAllMenuFunc;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryOporAllMenuFunc(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_查询操作员特殊菜单功能
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryOporSpecMenuFunc;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryOporSpecMenuFunc(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_查询全部菜单
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_QueryAllMenu;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_QueryAllMenu(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
# 原子_公共_权限客户端_获取角色类型
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_GetRoleType;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_GetRoleType(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    /* [获取表记录变量][公共_权限_角色表][{角色类型}][@角色类型#][{角色编号}=@角色编号# and {机构编号}=@机构编号#][4][@角色编号#,@机构编号#] */
    select role_type into v_role_type from db_pub.tb_perm_role where role_no=v_role_no and co_no=v_co_no limit 1;
    if FOUND_ROWS() = 0 then
                
        SET v_error_code = "pubA.21.34.4";
        if v_mysql_message <> "" then
            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","机构编号=",v_co_no),"#",v_mysql_message);
        else
            SET v_error_info = concat("角色编号=",v_role_no,",","机构编号=",v_co_no);
...
# 原子_公共_权限客户端_回收菜单功能权限
DELIMITER ;;
DROP PROCEDURE IF EXISTS pra_permclient_WithdrawMenuFunc;;
DELIMITER ;;
CREATE PROCEDURE pra_permclient_WithdrawMenuFunc(
    IN p_opor_co_no int,
    IN p_opor_no int,
    IN p_oper_mac varchar(32),
    IN p_oper_ip_addr varchar(32),
...
BEGIN
    ...
    if v_menu_type=1 then
        ...
        /* [获取表记录变量语句][公共_权限_角色表][group_concat({角色编号} SEPARATOR ';')][@角色编号串#][{机构编号}=@操作员机构编号# and ({角色类型}=《角色类型-交易》 or {角色类型}=《角色类型-交易特殊》)] */
        select group_concat(role_no SEPARATOR ';') into v_role_no_str from db_pub.tb_perm_role where co_no=v_opor_co_no and (role_type=1 or role_type=11) limit 1;


    /* elseif @菜单类型#=《菜单类型-管理》 then */
    elseif v_menu_type=2 then

        /* [获取表记录变量语句][公共_权限_角色表][group_concat({角色编号} SEPARATOR ';')][@角色编号串#][{机构编号}=@操作员机构编号# and ({角色类型}=《角色类型-管理》 or {角色类型}=《角色类型-管理特殊》)] */
        select group_concat(role_no SEPARATOR ';') into v_role_no_str from db_pub.tb_perm_role where co_no=v_opor_co_no and (role_type=2 or role_type=12) limit 1;


    /* elseif @菜单类型#=《菜单类型-运维》 then */
    elseif v_menu_type=3 then

        /* [获取表记录变量语句][公共_权限_角色表][group_concat({角色编号} SEPARATOR ';')][@角色编号串#][{机构编号}=@操作员机构编号# and ({角色类型}=《角色类型-运维》 or {角色类型}=《角色类型-运维特殊》)] */
        select group_concat(role_no SEPARATOR ';') into v_role_no_str from db_pub.tb_perm_role where co_no=v_opor_co_no and (role_type=3 or role_type=13) limit 1;


    /* elseif @菜单类型#=《菜单类型-服务》 then */
    elseif v_menu_type=4 then

        /* [获取表记录变量语句][公共_权限_角色表][group_concat({角色编号} SEPARATOR ';')][@角色编号串#][{机构编号}=@操作员机构编号# and ({角色类型}=《角色类型-服务》 or {角色类型}=《角色类型-服务特殊》)] */
        select group_concat(role_no SEPARATOR ';') into v_role_no_str from db_pub.tb_perm_role where co_no=v_opor_co_no and (role_type=4 or role_type=14) limit 1;
        ...
        if instr(v_menu_func_str, ";") > 0 then
            ...
            if instr(v_tmp_menu_func_str, ",") > 0 then
                ...
                if v_menu_type=1 then
                    ...
                    /* [删除表记录][公共_权限_角色交易菜单功能表][{业务菜单编号}=@业务菜单编号# and {业务功能编码}=@业务功能编码#][3][@角色编号#,@业务菜单编号#,@业务功能编码#] */
                    delete from db_pub.tb_perm_role_exch_menu 
                        where busi_menu_no=v_busi_menu_no and busi_func_code=v_busi_func_code;
                    if v_error_code = 1 then
                                
                        SET v_error_code = "pubA.21.51.3";
                        if v_mysql_message <> "" then
                            SET v_error_info = CONCAT(concat("角色编号=",v_role_no,",","业务菜单编号=",v_busi_menu_no,",","业务功能编码=",v_busi_func_code),"#",v_mysql_message);
                        else
                            SET v_error_info = concat("角色编号=",v_role_no,",","业务菜单编号=",v_busi_menu_no,",","业务功能编码=",v_busi_func_code);
                ...
                elseif v_menu_type=2 then
                    ...
                    /* [删除表记录][公共_权限_角色管理菜单功能表][{业务菜单编号}=@业务菜单编号# and {业务功能编码}=@业务功能编码#][3][@角色编号#,@业务菜单编号#,@业务功能编码#] */
                    delete from db_pub.tb_perm_role_mage_menu 
                        where busi_menu_no=v_busi_menu_no and busi_func_code=v_busi_func_code;
                    if v_error_code = 1 then
                ...
                elseif v_menu_type=3 then
                    ...
                    /* [删除表记录][公共_权限_角色运维菜单功能表][{业务菜单编号}=@业务菜单编号# and {业务功能编码}=@业务功能编码#][3][@角色编号#,@业务菜单编号#,@业务功能编码#] */
                    delete from db_pub.tb_perm_role_back_menu 
                        where busi_menu_no=v_busi_menu_no and busi_func_code=v_busi_func_code;
                    if v_error_code = 1 then
                ...
                elseif v_menu_type=4 then
                    ...
                    /* [删除表记录][公共_权限_角色服务菜单功能表][{业务菜单编号}=@业务菜单编号# and {业务功能编码}=@业务功能编码#][3][@角色编号#,@业务菜单编号#,@业务功能编码#] */
                    delete from db_pub.tb_perm_role_serv_menu 
                        where busi_menu_no=v_busi_menu_no and busi_func_code=v_busi_func_code;
                    if v_error_code = 1 then
