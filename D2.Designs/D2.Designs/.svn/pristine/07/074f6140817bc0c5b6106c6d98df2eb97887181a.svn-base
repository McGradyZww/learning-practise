...
API_EXPORT int LD_CALL_MODE QueryInterface(int iFunIndex, LPBIZ_FUNC_INFO bizFuncInfo)
{
    ...
    if (iFunIndex==0)
    {
        strcpy(bizFuncInfo->szFuncName, "clfutuL.1.21");
...
int iPriority=lpInBizMsg->GetInt32(LDTAG_PRIORITY);
    ...
    lpTSCall1Req1=glpFastMsgFactory->CreateFastMessage("clfutuT.1.21", 0);
    ...
    {
        lpOutBizMsg->SetInt32(LDTAG_ERRORNO, -1);
        lpOutBizMsg->SetString(LDTAG_ERRORINFO, "[clfutuT.1.21]subcall timed out!");
        iRet=-2;
        goto ENDSYS;
    }
    ...
    //if @错误编码// <> "0" then
    //    set @处理信息// = concat("数据归历史失败: 错误编码=", @错误编码//,"错误信息=", @错误信息//);
    //    [事务_清算期货_清算_记录处理信息][N]
    //end if;
