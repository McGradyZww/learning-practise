...
bool load_matrix(double* &pData, const char *file_name)
{
    ...
    pData = new double[(n+1)*block_size*group_size];
    for (int i=0; i<n; i++)
    {
        for (int j=0; j<n; j++)
        {
...
void print_line(double *pData, int line_num)
{
    ...
    if (my_rank == line_num)
    {
        ...
        for (int i=0; i<n+1; i++)
        {
            ...
            std::cout << element(pData, my_rank, i);
        }
        std::cout << std::endl;
    }
#endif
...
int main(int argc, char *argv[])
{
    ...
    if (my_rank == 0)
    {
        ...
        pBackUp = new double[(n+1)*block_size*group_size];
        memcpy(pBackUp, pData, (n+1)*block_size*group_size*sizeof(double));
    }
    else
    {
        single_recv(n, 0);
    ...
    pNodeData = pData + my_rank*block_size*(n+1);
    partialLine = new double[n+1];
    if (my_rank == 0)
    {
        for (int i=1; i<group_size; i++)
        {
            multi_send(pData+i*block_size*(n+1), block_size*(n+1), i);
        }
    }
    else
    {
        multi_recv(pNodeData, block_size*(n+1), 0);
    }
    //finished data initialize
    if (my_rank == 0)
    {
        std::cout << "before Jordan: " << std::endl;
        print_matrix(pData);
    }
    //sync();
    for (int k=0; k<n; k++)
    {
        ...
        if (my_rank == hostid)
        {
            ...
            single_broadcast(akk, 0, hostid);
            single_broadcast(akk, hostid+1, group_size);
            multi_broadcast(get_address(pNodeData, blockk, k), n-k+1, 0, hostid);
            multi_broadcast(get_address(pNodeData, blockk, k), n-k+1, hostid+1, group_size);
            #pragma omp parallel for
            for (int j=k; j<n+1; j++)
            {
//                std::cout << "thread " << omp_get_thread_num() << " echoed" << std::endl;
                element(pNodeData, blockk, j) /= akk;
            }
            #pragma omp parallel for
            for (int i=0; i<block_size; i++)
            {
                if (i != blockk)
                {
                    double aik = element(pNodeData, i, k);
                    for (int j=k; j<n+1; j++)
                    {
                        element(pNodeData, i, j) -= aik * element(pNodeData, blockk, j);
                    }
                }
            }
        }
        else
        {
            ...
            for (int i=0; i<block_size; i++)
            {
                ...
                for (int j=k; j<n+1; j++)
                {
                    element(pNodeData, i, j) -= aik * partialLine[j-k];
                }
            }
//            print_multi(partialLine, n-k, 2);
//            print_line(pData, 2);
//            print_line(pData, 2);
        }
    }
    if (my_rank == 0)
    {
        for (int i=1; i<group_size; i++)
        {
            multi_recv(pData + i*block_size*(n+1), block_size*(n+1), i);
        }
    }
    else
    {
        multi_send(pData + my_rank*block_size*(n+1), block_size*(n+1), 0);
    }
    if (my_rank == 0)
    {
        std::cout << std::endl;
    ...
    if (my_rank == 0)
    {
        ...
        for (int i=0; i<n; i++)
        {
            ...
            for (int j=0; j<n; j++)
            {
                tmp += element(pBackUp, i, j) * element(pData, j, n);
            }
            std::cout.width(8);
            std::cout.precision(4);
            std::cout.setf(std::ios::fixed);
